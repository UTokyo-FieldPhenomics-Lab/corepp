            ...
            with torch.no_grad():
                ...
                for jdx, item in enumerate(tqdm(iter(val_dataset))):
                    ...
                    if jdx % 10 == 0:
                        ...
                        frame_id = int(item['frame_id'][0])+1
                        frame_id_str = f'{frame_id:05d}'

                        # import ipdb;ipdb.set_trace()

                        base_path = os.path.join(param["data_dir"], item['fruit_id'][0], 'realsense/depth')
                        input_depth = o3d_utils.read_depth_as_pcd(os.path.join(base_path, frame_id_str + '.npy'))
